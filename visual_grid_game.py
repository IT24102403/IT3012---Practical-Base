...
class VisualGridHuntGame:
    ...
    def __init__(self, width=10, height=10, num_food=10, num_opponents=2, custom_walls=None):
        ...
        self.agent_dir = 'Up'  # Tracks the direction the agent is currently facing(lab02)
        ...
        # ==================== NEW CODE STARTS HERE lab01 ====================
        # Inside VisualGridHuntGame.__init__()
        ...
        while len(self.toxic_traps) < num_traps:
            ...
            pos = (tx, ty)
            # Ensure trap does not spawn on start position (0,0), walls, or food
            if (
              pos != (0, 0)
    ...
    def get_percept(self) -> dict:
        return {
            ...
            'smells_toxin': tuple(self.agent_pos) in self.toxic_traps,  # NEW SENSOR KEY lab01
    ...
    def get_percept(self) -> dict:
        """Returns local sensory booleans instead of global position coordinates for Lab 02[cite: 7]."""
        x, y = self.agent_pos

        # 1. Calculate the coordinates of the tile directly ahead based on current direction[cite: 7]
        ...
        # 2. Check if the tile ahead is beyond grid borders or hits a wall[cite: 7]
        out_of_bounds = (next_x < 0 or next_x >= self.width or next_y < 0 or next_y >= self.height)
        hits_wall_tile = (next_x, next_y) in self.walls
        wall_ahead = out_of_bounds or hits_wall_tile

        # 3. Return local sensory input dictionary[cite: 7]
        return {
...
class GridGameGUI:
    ...
    def draw_grid(self):
        ...
        # 2. In GridGameGUI.draw_grid(self):lab01
        for tx, ty in self.env.toxic_traps:
            offset = self.cell_size * 0.2
            x1 = tx * self.cell_size + offset
            y1 = (self.env.height - 1 - ty) * self.cell_size + offset
            # Render custom purple square for traps
